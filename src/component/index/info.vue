<template>
  <div :class="{ info: true, open, tall }" @click="$emit('touched', 'geo', $event)">
    <div class="panel" @click.stop="$emit('touched', 'info:background', $event)">
      <div class="handlebar">
        <div class="handle" @click.stop="$emit('touched', 'info:handle', $event)">
          <h3 v-if="entry.title" class="handle-title">{{ entry.title }} <ui-icon v-if="entry.featured" name="star" color="#d9d9d9" size="1.6rem"></ui-icon></h3>
